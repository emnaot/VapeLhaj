    ...
    return (
      ...
      <h2 className='text-3xl font-extrabold text-black antialiased tracking-tight py-4'>{heading}</h2>
            ...
            <div className='flex items-center gap-4 md:gap-6 overflow-x-scroll scrollbar-none transition-all' ref={scrollElement}>
                ...
                <button className='bg-gold-dark shadow-md rounded-full p-1 absolute right-0 text-lg hidden md:block' onClick={scrollRight}>
                ...
                {loading ? (
                    loadingList.map((_, index) => (
                        <div key={index} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow-md'>
                            <div className='bg-gray-300 h-48 p-4 min-w-[280px] md:min-w-[145px] flex justify-center items-center animate-pulse'></div>
                    ...
                    data.map((product, index) => (
                        <Link key={index} to={"product/" + product?._id} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow'>
                            ...
                            <div className='p-4 grid gap-3'>
                                ...
                                <button className='text-sm bg-gold hover:bg-gold-dark text-white px-3 py-1 rounded-full transition-colors' onClick={(e) => handleAddToCart(e, product?._id)}>Add to Cart</button>
                            </div>
                        </Link>
                    ))
                )}
